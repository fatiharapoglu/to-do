...
class Storage {
    static setProjectsToStorage(projects) {
        // basicly saves "wrapper" everytime DOM renders
    ...
    static loadFromStorage() {
        // the saved storage is an object which is useless if it's not a class prototype of the related tasks and projects
        let newWrapper = new Wrap();
        let retrievedProjects = this.getProjectsFromStorage();
        if (retrievedProjects === null) {
            DOM.pushDefault();
        } else {
            // so we have to assign classes to all stored objects to use related methods
            newWrapper.setProjectList(retrievedProjects);
            newWrapper.setProjectList(
                newWrapper.getProjectList().map((project) => Object.assign(new Project(), project))
            );
            newWrapper
                .getProjectList()
                .forEach((project) =>
    ...
    static setTheme(mode) {
        // set color theme from DOM module
    ...
    static getTheme() {
        if (localStorage.getItem("theme") === null || localStorage.getItem("theme") == "dark") {
            ...
            // else change the theme from DOM module
            DOM.themeChanger();
        }
    }
}
